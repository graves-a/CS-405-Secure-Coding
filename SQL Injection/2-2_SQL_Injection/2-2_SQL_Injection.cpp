...
bool run_query(sqlite3* db, const std::string& sql, std::vector< user_record >& records)
{
    ...
    //  something more generic
     ...
     // convert sql to a non const string so we can edit it and look for "or"
     /*std::string newQuery = sql;
     std::string bad = "or";

     if (newQuery.find(bad) != std::string::npos)
     {
         newQuery.erase(newQuery.find(bad), bad.length());
     }*/

    std::string newQuery;
    std::string originalQuery = sql;
    const std::regex regexReplace("(or).*$");                             // selects substring starting at 'or' to the end of the string 
    const std::regex regexMatch(".*(or [\\t ]*(.+)=\\2(?:[ \\t]*|$)).*"); // looks for a substring starting with 'or' and including true = true, ignores cases where they are inequal
    std::cout << std::regex_match(originalQuery, regexMatch) << std::endl;
    
    if (std::regex_match(originalQuery, regexMatch)) {
        newQuery = std::regex_replace(originalQuery, regexReplace, "SQL Injection Detected");
        std::cout << newQuery << std::endl;
    }
    else {
        std::cout << "No SQL Injection attempt detected" << std::endl;
    ...
    char* error_message;
    if (sqlite3_exec(db, newQuery.c_str(), callback, &records, &error_message) != SQLITE_OK)
    {
